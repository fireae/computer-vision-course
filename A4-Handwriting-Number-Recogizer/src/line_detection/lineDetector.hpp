...
class houghLineSpace {
    private:
		...
		int height;
    public:
		houghLineSpace(int radiusSize, int thetaSize=360, int scaleR=1, int scaleT=1, int maxPLen = 32) {
		...
		void clusterLines(unsigned char *data) {
			...
			// Delete lines too close
			for (auto it=lines.begin(); it != lines.end(); ) {
				bool del = false;
				for (int l = (it-lines.begin()+1); l < lines.size(); l++) {
					double diff = abs(lines[l].theta-it->theta) + abs(lines[l].radius-it->radius);
					if (diff < 10) {
						del = true;
					}
				}
			...
			// TODO: Add lines for number of lines less than 4.
			...
			for (int l1 = 0; l1 < lines.size()-1; l1++) {
				for (int l2 = l1+1; l2 < lines.size(); l2++) {
					...
					double t1 = lines[l1].theta,
							...
							r2 = lines[l2].radius;
					if (sin(t1-t2) != 0) {
						double w1 = (sin(t1)*r2 - sin(t2)*r1) / sin(t1-t2);
						double h1 = (cos(t2)*r1 - cos(t1)*r2) / sin(t1-t2);
...
class lineDetector {
	...
	public:
		lineDetector(CImg<unsigned char> *rgbImg) {
			double scaled_size = 1000.0;
			this->scale = scaled_size/(double)(rgbImg->height());
			if (rgbImg->height() < rgbImg->width())
				this->scale = scaled_size/(double)(rgbImg->width());
		...
		void toEdge(CImg<unsigned char> &img) {
			unsigned char* data = (unsigned char*)malloc(img.height() * img.width() * sizeof(unsigned char));
			memset(data, 0, img.height() * img.width() * sizeof(unsigned char));

			// preprocess the image.
			for (int h = 0; h < img.height(); h++) {
				for (int w = 0; w < img.width(); w++) {
					int index = h*img.width() + w;
					data[index] = (img(w, h, 0) + img(w, h, 1) + img(w, h, 2))/3;
				}
			}
			// cannylit myCanny(data, img.width(), img.height(), 2, 6, 3.5, 16);
			cannylit myCanny(data, img.width(), img.height(), 5, 9, 8, 16);
			memcpy(data, myCanny.edgeData(), img.height() * img.width());

			img = CImg<unsigned char>(data, img.width(), img.height());
			// img.display();
		}
		~lineDetector() {
			free(this->data);
