...
class GT {
    ...
    public:
        ...
        int get_threshold(CImg<unsigned char> gimg) {
            updateHistogram(gimg);  // 更新灰度直方图
            // 遍历计算类间方差
            // 迭代找T
            int i = 0;
        ...
        double nextT() {
            ...
            for (int p = 0; p < T; p++) {
            if (sum1) T1 += (double)histogram[p]/sum1 * p;
            else T1 += (double)histogram[p] * p;
            }
            for (int p = 255; p >= T; p--) {
                if (sum2) T2 += (double)histogram[p]/sum2 * p;
                else T2 += (double)histogram[p] * p;
            }
            return (T1+T2) / 2;
        }
};
