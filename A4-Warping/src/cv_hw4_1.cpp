...
void warpingShear(CImg<unsigned char> *rgbImg, CImg<unsigned char> *warpedImg, vector<double> wp, vector<double> hp) {
...
void warpingAffine(CImg<unsigned char> *rgbImg, CImg<unsigned char> *warpedImg, vector<double> wp, vector<double> hp) {
	// Get four corners and warped paper size.
	point nw, ne, sw, se;
	getNSEW(wp, hp, nw, ne, sw, se);
	// ne = point(ne.w-nw.w, ne.h-nw.h);
	// sw = point(sw.w-nw.w, sw.h-nw.h);
	// se = point(se.w-nw.w, se.h-nw.h);
	...
	trans_matrix T1, T2;
	T1.solveAffine(nw_, ne_, sw_, nw, ne, sw);
	T2.solveAffine(ne_, sw_, se_, ne, sw, se);
	// T.c += nw.w;
	// T.f += nw.h;
	...
	for (int h = 0; h < h_size; h++) {
		...
		for (int w = 0; w < crossLine; w++) {
			point f_point = T1.mul(point(w, h));
			getColor(rgbImg, f_point, warpedImg, w, h);
		}

		for (int w = crossLine; w < w_size; w++) {
			point f_point = T2.mul(point(w, h));
			getColor(rgbImg, f_point, warpedImg, w, h);
		}
	}

}
void warpingProjective(CImg<unsigned char> *rgbImg, CImg<unsigned char> *warpedImg, vector<double> wp, vector<double> hp) {
	// Get four corners and warped paper size.
	point nw, ne, sw, se;
	getNSEW(wp, hp, nw, ne, sw, se);
	// ne = point(ne.w-nw.w, ne.h-nw.h);
...
int main() {
	...
	for (int i = 0; i < 6; i++) {
		...
		printf("\n[%d] Line detecting costs %f sec.  \n", i, (double)(end - start) / CLOCKS_PER_SEC);
		
		
		// Shear Warp image
		start = clock();
		CImg<unsigned char> outImg;
		warpingShear(&rgbImg, &outImg, w, h);
		end = clock();
		printf("Shear Warping costs %f sec. \n", (double)(end - start) / CLOCKS_PER_SEC);
		string outpath = outputPath + "shear/output" + pictureName[i] + ".bmp";
		outImg.save(outpath.c_str());


		// Affine Warp image
		start = clock();
		warpingAffine(&rgbImg, &outImg, w, h);
		end = clock();
		printf("Affine Warping costs %f sec. \n", (double)(end - start) / CLOCKS_PER_SEC);
		outpath = outputPath + "affine/output" + pictureName[i] + ".bmp";
		outImg.save(outpath.c_str());


		// Projective Warp image
		start = clock();
		warpingProjective(&rgbImg, &outImg, w, h);
		end = clock();
		printf("Projective Warping costs %f sec. \n", (double)(end - start) / CLOCKS_PER_SEC);
		outpath = outputPath + "projective/output" + pictureName[i] + ".bmp";
		outImg.save(outpath.c_str());
